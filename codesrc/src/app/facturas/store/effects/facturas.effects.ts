...
export class FacturaEffects {
  ...
  @Effect()
  public getAllfacturas$ : Observable<Action> = this.actions$.pipe(
      ofType<GetAllFacturas>(facturaActions.GET_FACTURAS),
        mergeMap((action:GetAllFacturas) =>
          this.svc.findAll().pipe(
              map((facturas: Factura[]) => new GetAllFacturasSuccess(facturas)),
                catchError(err => of(new GetAllFacturasError(err)))
          )
      )
  );
      ...
      ofType<AddFactura >(facturaActions.CREATE_FACTURA),
        mergeMap((action:AddFactura ) =>
          this.svc.insert(action.payload).pipe(
              map((factura:Factura) => new AddFacturaSuccess(factura.id)),
      ...
      ofType<UpdateFactura>(facturaActions.UPDATE_FACTURA),
        mergeMap((action:UpdateFactura) =>
          this.svc.update(action.payload).pipe(
            map((factura:Factura) => new UpdateFacturaSuccess()),
