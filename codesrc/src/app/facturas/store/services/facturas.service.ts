...
export class FacturasService {
   ...
   */
   public insert(data: Factura): Observable<Factura>{
     ...
     .pipe(
        tap((newFactura:Factura) => this.traceService.log(`added factura w/ id=${newFactura.factId}`)),
   ...
   public update(factura: Factura): Observable<Factura> {
     let headers = new HttpHeaders();
     headers = headers.set('Content-Type', 'application/json; charset=utf-8');
     return this.http.put<Factura>(this.URL + '/' + factura.factId, factura, {headers: headers}).pipe(
