...
import { GetAllFacturas } from './store/actions/facturas.actions';
import { isCreated , isUpdated , isDeleted ,getDeleteError ,getUpdateError , getFacturasError , getCreateError } from './store/reducers/facturas.reducers';
...
export class FacturasComponent implements OnInit {
  ...
  ngOnInit(){
    ...
    this.store.dispatch(new GetAllFacturas());
    this.store.select(getFacturasError).subscribe((error) => this.loadingError(error));
